...
import { PrismaClient, TableStatus, Prisma } from '@prisma/client';
...
const validStatuses = ['AVAILABLE', 'OCCUPIED', 'RESERVED', 'CLEANING'] as const;
type ValidStatus = typeof validStatuses[number];

// Helper to safely parse and cast status strings to TableStatus enum
function parseTableStatus(input?: string, fallback?: TableStatus): TableStatus {
  if (input && validStatuses.includes(input as ValidStatus)) {
    return input as TableStatus;
  }
...
router.post(
  ...
  asyncHandler(async (req: Request, res: Response) => {
    ...
    const statusInput = req.body.status as string | undefined;
    ...
    const status = parseTableStatus(statusInput);
...
router.put(
  ...
  asyncHandler(async (req: Request, res: Response) => {
    ...
    const status = parseTableStatus(statusInput, existingTable.status);
    ...
    const table = await prisma.table.update({
      ...
      data: {
        number,
        capacity,
        status: { set: status } as Prisma.EnumTableStatusFieldUpdateOperationsInput,
      },
    });
